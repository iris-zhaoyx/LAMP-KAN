...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    # Load scheduler, tokenizer and models.     加载基础模型组件
    ...
    # Freeze vae and text_encoder 冻结非必要模块
    vae.requires_grad_(False)                         # 特征提取器无需更新
    text_encoder.requires_grad_(False)                # CLIP文本编码器已预训练充分

    unet.requires_grad_(False)                        # 仅更新指定注意力层的查询矩阵
    for name, module in unet.named_modules():            #检查UNet 的所有子模块名称是否以 trainable_modules 列表中的任一后缀结尾。若匹配成功，则将其所有参数设为可训练
    ...
    if enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            unet.enable_xformers_memory_efficient_attention()                   # 替换原生注意力为Flash Attention
        else:
            raise ValueError("xformers is not available. Make sure it is installed correctly")

    if gradient_checkpointing:
        unet.enable_gradient_checkpointing()                        # 牺牲少量时间换取显存降低
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):        # 梯度累积上下文
                ...
                pixel_values = rearrange(pixel_values, "b f c h w -> (b f) c h w")            # 展平时间维
                latents = vae.encode(pixel_values).latent_dist.sample()                       # VAE编码
                latents = rearrange(latents, "(b f) c h w -> b c f h w", f=video_length)      # 恢复时间维
                latents = latents * 0.18215                # 标准化到[-1,1]
                ...
                # 入视频的潜码形状为 (B, C, T, H, W)，其中：B: Batch size（批量大小），C: Channels（通道数，如 RGB=3），T: Time steps（时间步/帧数），H: Height（图像高度），W: Width（图像宽度）
                # 对每个样本的每一张图像通道，将其第一帧的特征值完整复制到 noisy_latents 的对应位置
                noisy_latents[:, :, 0:1, :, :] = latents[:, :, 0:1, :, :]
                # 执行 noisy_latents[...] = latents[...] 后：第一帧的所有像素（t=0）会被强制设为原始值（A, C）。其他帧（t≥1）已被之前的 noise_scheduler.add_noise() 添加了噪声。
